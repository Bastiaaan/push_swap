...
/*   Updated: 2026/01/27 17:44:36 by brogaar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	finish(t_list *a, t_list *b, int borrowed)
{
	while (borrowed > 0)
	{
		if (ft_lstsize(b) >= 2 && b->content < b->next->content)
			sb(&b);
		else if (b->content > a->content && b->content < a->next->content)
			ra(&a);
		else if (b->content < a->content && b->content < ft_lstlast(a)->content
			&& ft_lstlast(a)->content < a->content)
			rra(&a);
		else if (b->content < a->content && (a->content < ft_lstlast(a)->content
...
void	mini_sort(t_list *a, t_list *b)
{
	...
	while (!ascending(a))
	{
		if (ft_lstsize(a) >= 2 && a->content > a->next->content && !exceed_largest(a, a))
			sa(&a);
